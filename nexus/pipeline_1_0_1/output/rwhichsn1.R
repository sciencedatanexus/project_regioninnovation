...
labels_markers <- sprintf( # Label for markers (org ID card)
  "<strong>%s</strong><br/>
  %s records<br/>
  %s researchers<br/>
  %s inventors<br/>
  ...
  R&D strength: %s",
  nodes_map$name,
  format(nodes_map$nb_records, big.mark = ",", scientific = FALSE),
  ceiling(nodes_map$nb_records/5),
  ceiling(nodes_map$nb_records/20),
  ceiling(nodes_map$nb_records/10),
  ceiling(nodes_map$nb_records/30),
  ceiling(nodes_map$nb_records/40),
  ceiling(nodes_map$nb_records/50)
)

hover_markers <- labels_markers %>% lapply(htmltools::HTML) # Label for markers (org ID card) for mouse hover

popup_markers <- sprintf( # Label for markers (org ID card) for mouse click (adds link)
  paste0(labels_markers,"<br/>Review Profile"),
  nodes_map$org_id
) %>% lapply(htmltools::HTML)
...
eda_plot <- leaflet(country_data) %>%
  ...
  addMarkers(
    label = hover_markers,
    popup = popup_markers,
    options = markerOptions(interactive = TRUE, title = nodes_map$name, alt = nodes_map$name, riseOnHover = TRUE),
...
data_stats <- tibble(
        distance_type = c("local", "national", "international"),
        nb_orgs = c(max_rec/5, max_rec/(5/2), max_rec/(5/3))
)
# If there are additional output steps, they would go here. For example, saving
# more visualizations, or generating a report.
return(list(net = eda_plot, table = data_table, hero=total_nb, stats = data_stats))
# Diconnect to DB (at the end to prevent Connection to be garbage-collected)
dbDisconnect(conn, shutdown = TRUE)
